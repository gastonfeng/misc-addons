...
class DeliveryGrid(models.Model):
    ...
    def get_price(self,  id, order, dt, context=None):
        total = 0
        weight = 0
        volume = 0
        quantity = 0
        ...
        for line in order.order_line:
            if not line.product_id or line.is_delivery:
                continue
            q = product_uom_obj._compute_qty( line.product_uom.id, line.product_uom_qty, line.product_id.uom_id.id)
        ...
        return self.get_price_from_picking( id, total, weight, volume, quantity, special_delivery, context=context)

    def get_price_from_picking(self,  id, total, weight, volume, quantity, special_delivery=0, context=None):
        grid = self.browse( id, context=context)
        price = 0.0
        ok = False
