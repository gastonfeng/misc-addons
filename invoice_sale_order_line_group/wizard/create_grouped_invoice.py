...
class GroupedInvoiceWizard(osv.osv_memory):
    ...
    def make_grouped_invoice(self,  ids, context=None):
        ...
        for line in sales_order_line_obj.browse( context.get('active_ids', []), context=context):
            if (not line.invoiced) and (line.state not in ('draft', 'cancel')):
                inv_line_id = sales_order_line_obj.invoice_line_create( [line.id])
        ...
        partner_currency = {}
        for o in order_obj.browse( orders, context=context):
        ...
        for val in invoices.values():
            res = order_obj._make_invoice( val[0][0], reduce(lambda x, y: x + y, [l for o, l in val], []), context=context)
            invoice_ref = ''
            origin_ref = ''
            for o in set([o for o, l in val]):
                invoice_ref += (o.client_order_ref or o.name) + ' | '
                origin_ref += (o.origin or o.name) + ' | '
                order_obj.write( [o.id], {'state': 'progress'})
                cr.execute('insert into sale_order_invoice_rel (order_id,invoice_id) values (%s,%s)', (o.id, res))
            order_obj.invalidate_cache( ['invoice_ids'], [o.id], context=context)
        ...
        if context.get('open_invoices', False):
            return self.open_invoices( ids, created_ids, context=context)
        return {'type': 'ir.actions.act_window_close'}

    def open_invoices(self,  ids, invoice_ids, context=None):
