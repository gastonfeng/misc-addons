...
class WebsiteBookingCalendar(Controller):
    ...
    def _get_resources(self, params):
        ...
        if not venue_id:
            venues = venue_obj.search( [], context=context)
            venue_id = venues[0] if venues else None
        if pitch_id:
            return pitch_obj.browse( int(pitch_id), context=context).resource_id
        elif venue_id:
            pitch_ids = pitch_obj.search( [('venue_id', '=', int(venue_id))], context=context)
    ...
    def _get_values(self, params):
        ...
        venue_obj = request.registry['pitch_booking.venue']
        venues = venue_obj.browse( venue_obj.search( [], context=context), context=context)
        values.update({
            'venues': venues,
